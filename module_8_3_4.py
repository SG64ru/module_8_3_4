class Car:
    ...
    def __is_valid_vin(self, vin_number):
        ...
        if self.vin_number <= 1000000 or self.vin_number >= 9999999: # Почему не срабатывает????
            raise IncorrectVinNumber("Неверный диапозон VIN номера")
        # if self.vin_number <= 1000000: # Почему не срабатывает????
        #     raise IncorrectVinNumber("Неверный диапозон VIN номера")
        # if self.vin_number >= 9999999: # Почему не срабатывает????
        #     raise IncorrectVinNumber("Неверный диапозон VIN номера")
